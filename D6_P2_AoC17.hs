...
reallocate memBanks seenBefore cycleNum
  | elem  memBanks seenBefore == True = findDistance memBanks (seenBefore ++ [memBanks])
...
findDistance memBanks seenBefore =
  let distance = elemIndices memBanks seenBefore
  in  (distance !! 1) - (distance !! 0)
